...
class SingleItemGenerationDataModule(pl.LightningDataModule):
    ...
    def statistics(self) -> DatasetStatistics:
        ...
        image_size = None
        for batch in tqdm(enumerate(self.train_dataloader()), desc="Asserting types and image sizes"):
            ...
            assert object_mask.shape[1] == image_size[0] and rgb_with_object.shape[2] == image_size[1]
        ...
        for i, batch in tqdm(self.train_dataloader(), desc="Calculating dataset var"):
            rgb = batch["model_input"]["rgb"]
            batch_num_samples = rgb.shape[0]
            batch_var = ((rgb - dataset_mean) ** 2).mean(dim=[0, 1])
            dataset_var = (dataset_mean * samples_included_in_var + batch_num_samples * batch_var) / (
